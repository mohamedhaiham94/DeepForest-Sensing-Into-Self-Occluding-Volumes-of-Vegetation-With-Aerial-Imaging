...
def correct_lens_distortion_sequoia(meta, image):
    ...
    FisheyePoly = np.array(meta.get_item('XMP:FisheyePolynomial').split(',')).astype(float)
    p0 = FisheyePoly[0]
    p1 = FisheyePoly[1]
    p2 = FisheyePoly[2]
    p3 = FisheyePoly[3]

    FisheyeAffineMat = np.array(meta.get_item('XMP:FisheyeAffineMatrix').split(',')).astype(float)
    ...
    # undistortedImage = cv2.remap(image, XMap, YMap, cv2.INTER_LINEAR)
